    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException, IOException {
        try {
            ...
            if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
                String tokenHash = tokenProvider.getTokenHashFromJWT(jwt);

                /*
                    Note that you could also encoders the users's username and roles inside JWT claims
            ...
            logger.error("Could not set users authentication in security context", ex);
        }
